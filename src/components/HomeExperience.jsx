...
import { Roci } from './canvas/Rocinante'
...
const CURVE_AHEAD_ROCI = 0.02
const ROCI_MAX_ANGLE = 35
...
We have a wide range of beverages!`,
  ...
  useFrame((_state, delta) => {
    ...
    const tangent = curve.getTangent(lerpedScrollOffset + CURVE_AHEAD_ROCI)
    ...
    // LIMIT SHIP ANGLE
    if (angleDegrees < 0) {
      angleDegrees = Math.max(angleDegrees, -ROCI_MAX_ANGLE)
    }
    if (angleDegrees > 0) {
      angleDegrees = Math.min(angleDegrees, ROCI_MAX_ANGLE)
    }

    // SET BACK ANGLE
    angle = (angleDegrees * Math.PI) / 180

    const targetRociQuaternion = new THREE.Quaternion().setFromEuler(
      new THREE.Euler(
        rocinante.current.rotation.x,
        rocinante.current.rotation.y,
        angle
      )
    )

    rocinante.current.quaternion.slerp(targetRociQuaternion, delta * 2)
  ...
  useLayoutEffect(() => {
    ...
    shipInTl.current.from(rocinante.current.position, {
      duration: 3,
      z: 5,
      y: -2,
    })
    ...
    shipOutTl.current.to(rocinante.current.position, {
      duration: 1,
      z: -1000,
    })
  }, [])
    ...
    () => (
      <>
        ...
        <group ref={cameraGroup}>
          <Speed />
          <HomeBG backgroundColors={backgroundColors} />
